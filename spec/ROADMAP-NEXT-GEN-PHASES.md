# Roadmap — Phases 8 through 16 (post-KCD-Texas)

**Scope:** the next-generation expansion of "The 90-Minute IDP" beyond the 7-phase / 27-component build shipped at KCD Texas 2026 on 2026-05-15. The KCD Texas spec (`spec/BUILD-SPEC.md`) remains canonical for that workshop run; this document is the forward-looking roadmap for the next iteration(s).

**Status:** Addendum to the original Phase 8+ roadmap. Locks in confirmed additions, drops multi-cluster federation, and adds service mesh as a standalone phase.

---

## Phase 13–16 Additions: Identity, Secrets, and Service Mesh

## Updated final ranking (supersedes the original TL;DR)

Build phases in this order. Bold = confirmed by Michael.
...
**Dropped from the original roadmap:** multi-cluster federation (Karmada, Open Cluster Management, Cilium Cluster Mesh). Out of scope.
...
SPIFFE solves this with short-lived X.509 SVIDs (Secure Verifiable Identity Documents) and JWT-SVIDs, both backed by workload attestation that's tied to the runtime (Kubernetes labels, container image hashes, kernel-verified PID lineage). Federation across trust domains is built into the spec.
...
This is genuinely operational, not theoretical — Bloomberg, Square, Pinterest, ByteDance, Uber, and Netflix run this at scale.
...
You currently have no human-identity story beyond cloud IAM. Developers access ArgoCD via... what? kubectl via long-lived service account tokens? Backstage with no auth? This is the most common gap in DIY platforms.
...
- **OAuth2 Proxy** as the per-app authenticator for components that don't speak OIDC natively
...
The real self-service value isn't "log in via SSO" — it's **time-bounded privilege escalation through a Backstage form**:
...
Dex is in production at: Mojang/Minecraft (Microsoft), Pusher, LightStep, eBay, RedHat OpenShift's identity broker. Pinterest published a detailed account of their Dex-based JIT access platform in 2024. Keycloak is the more enterprise-common choice — used at every large company that runs its own IdP.
...
You have External Secrets Operator (Phase 3), which is excellent — but ESO is a sync mechanism, not a secrets store. Today it pulls from AWS Secrets Manager. The gaps:

- **Cluster-portability:** AWS Secrets Manager doesn't exist on kubeadm. Your platform is non-portable.
...
- **PKI engine:** cert-manager needs a CA. You're currently using self-signed or Let's Encrypt. A real platform has its own internal CA.
- **Encryption-as-a-service:** Apps that need to encrypt PII shouldn't manage keys themselves.

**Vault would solve all this — but Vault went BUSL in 2023.** It's no longer open-source. OpenBao is the Linux Foundation fork from before the license change, Apache 2.0, with a real community.
...
This is genuinely novel for 2026 and worth highlighting:
...
**Phase 3 (Security/ESO):** ESO's OpenBao backend replaces ESO's AWS Secrets Manager backend (or runs alongside)
...
This is the most operationally complex addition in the roadmap. OpenBao requires:
...
If you're EKS-only and AWS Secrets Manager + IRSA already meets your needs, the marginal value of OpenBao is the dynamic-secrets and PKI capabilities. If your workloads are mostly long-running services with rarely-rotating credentials, the operational overhead may exceed the benefit. **Michael's case: confirmed worth doing** because the workshop targets multi-cluster (EKS + kubeadm) and demonstrates AI-native patterns where dynamic per-request secrets matter.
...
You have north-south traffic management via kgateway (Phase 8). You do not currently have east-west: service-to-service mTLS, identity-aware authorization between workloads, traffic split for canaries, cross-cluster service discovery. A mesh fills that gap.

**Don't add a mesh just because.** Add it because:
...
If your platform is mostly N-S (agent calls external LLM via kgateway, done), you may not need a mesh at all. But once Phase 8 + Phase 9 + Phase 13 land, the E-W story matters.
...
- You need rich L7 request transformation (header manipulation, JWT claim-based routing, complex authz with CEL)
- You have a team already operating Istio successfully and don't want to switch
- You need the broader ecosystem of community Envoy filters (e.g., ext_authz to a centralized policy engine, custom rate limiting)
- You need Istio's WasmPlugin extensibility model
- You're building a service mesh-of-record for a large organization (50+ services) where the feature ceiling matters more than operational simplicity

Istio Ambient is materially better than 2024-era sidecar Istio. The ztunnel-only mode is comparable to Cilium in operational simplicity for L4 mTLS. Waypoints are opt-in per namespace, so the L7 complexity is bounded to where you actually need it.
...
- **Gateway API** support for HTTPRoute (parallel to kgateway's, scoped to E-W if you go that route — or just let kgateway own all Gateway API resources and Cilium handle pure E-W policy)
...
**Phase 8 (Agent Gateway):** kgateway handles N-S, mesh handles E-W. Clean separation. If you go Istio Ambient, you'd need to decide between kgateway and Istio Gateway for N-S — but Cilium has no Gateway competition so the integration is simpler.
...
Cilium: **Low to medium.** ClusterMesh-free deployment is straightforward. Cross-cluster service discovery would require ClusterMesh, which Michael has dropped from scope — so single-cluster mesh in EKS, single-cluster mesh in kubeadm, no cross-cluster E-W. Fine.
...
- `phase-16-service-mesh/cilium-mesh/` — Cilium upgrade values.yaml enabling mesh features (assumes Cilium is the CNI; if you're on a non-Cilium CNI, this phase requires CNI replacement, which is a separate concern)
...
- **Multi-cluster federation (Karmada, OCM)** — out of scope per the latest decision
- **Cilium Cluster Mesh** — out of scope (multi-cluster dropped)
- **HashiCorp Vault** — BUSL. Use OpenBao instead (Phase 15)
- **OPA / Gatekeeper** — Kyverno is CEL-based and graduated. Don't double up.
...
- **Vitess** — overkill unless you specifically teach MySQL sharding
- **KubeEdge** — no edge use case in scope

---

## Updated synthesis: the AI-native, identity-first, self-service IDP
...
With those three, you can credibly claim AI-native and identity-first. Everything else is enrichment.
