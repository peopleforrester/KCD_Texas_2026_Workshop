...
- `.claude/commands/build-phase.md` — `/build-phase N`, a **catch-up / fallback** for a single phase. Reads spec → skill → generates manifest to `~/my-<component>.yaml` → diffs against `gitops/apps/` ground truth → runs `pytest tests/test_phase_0N_*.py` → emits `<promise>PHASE_N_DONE</promise>` (or `<promise>PHASE_N_FAILED</promise>` — honest failures only). The **primary workflow** is single-paste autonomous execution from `spec/BUILD-SPEC.md`; this command is the per-phase fallback when the autonomous loop gets stuck or a student needs to catch up.
...
- `.claude/hooks/cc-stop-deterministic.sh` + `.claude/settings.json` — workshop stop hook. Blocks Claude from exiting until the phase emits `<promise>PHASE_N_DONE</promise>` or `<promise>ALL_PHASES_DONE</promise>`. Inactive unless the marker file `.build-active` exists.
- `tests/test_phase_0[1-4]_*.py` — pytest test gates per phase. Real kubectl calls via `tests/conftest.py` fixtures, no mocks. Each phase's spec block names the test file the gate runs.
- `.pre-commit-config.yaml` — local pre-commit hooks: gitleaks, yamllint, kubeconform (K8s schema validation against real CRDs), helm-lint, shellcheck, dry-run validator on pre-push, conventional commit format. Run `pre-commit install` once to wire them up.
- `spec/BRANCH-WORKFLOW.md` — single-maintainer staging→main workflow doc. ArgoCD reads from `main`, edits land on `staging` first, dry-run validator must pass on pre-push.
