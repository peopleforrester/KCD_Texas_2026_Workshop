...
- `kcd-texas-student-playbook.md` — student-facing 90-minute walkthrough. Spec-driven build via `/workshop-phase 1..4`; Tour mode is the fallback. Modeled on `kubeauto-ai-day/spec/BUILD-SPEC.md` but condensed for workshop pacing per `assets/kubeauto-reference-analysis.md`.
- `spec/WORKSHOP-BUILD-SPEC.md` — the 4-phase condensed spec the `/workshop-phase` command reads. Documents the workshop scope, per-phase outputs, verify commands, and a **Known correction patterns** table that lists the bugs the kubeauto reference build hit so the workshop doesn't repeat them.
- `.claude/skills/{argocd-patterns,kyverno-policies,backstage-templates}.md` — workshop-adapted copies of three kubeauto skill files. Each encodes version pins, value paths, and correction patterns Claude reads before generating a manifest. Critical workshop-specific corrections baked in: `configs.cm` (not `configs.params`) for the ArgoCD reconciliation timeout; chart 9.x (not 7.x) for current ArgoCD 3.3.x; Kyverno webhook `namespaceSelector` as a map (not list); Backstage `createBackend()` (not legacy `createServiceBuilder`).
- `.claude/commands/workshop-phase.md` — defines `/workshop-phase N`. Read context → build (or tour fallback) → verify with kubectl → score → emit `<promise>WORKSHOP_PHASE_N_DONE</promise>`. The skill files and reference manifests are the substrate.
- `.claude/hooks/cc-stop-deterministic.sh` + `.claude/settings.json` — workshop stop hook. Blocks Claude from exiting a `/workshop-phase` until the completion promise fires. Inactive unless `.workshop-active` exists (created by the command).
...
- `gitops/` — the GitOps source ArgoCD watches on each student cluster.
  ...
  - `gitops/apps/` — five child Applications (Kyverno wave -5, kyverno-policies wave -4, kube-prometheus-stack wave 1, argocd-servicemonitors wave 2, Backstage wave 5), each with sync-wave annotations matching the kubeauto reference build's ordering.
  ...
  - `gitops/manifests/argocd-servicemonitors/` — three ServiceMonitor manifests targeting ArgoCD's server, application-controller, and repo-server metrics services.  Wave 2 so they apply *after* kube-prometheus-stack (wave 1) registers the ServiceMonitor CRD.  Requires the Phase 1 Helm install to enable `controller.metrics.enabled / server.metrics.enabled / repoServer.metrics.enabled` so the metrics Services exist.  Also requires `serviceMonitorSelectorNilUsesHelmValues: false` on kube-prom-stack so Prometheus picks them up.
