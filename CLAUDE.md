...
- `.claude/hooks/cc-stop-deterministic.sh` + `.claude/settings.json` — workshop stop hook. Blocks Claude from exiting until the phase emits its promise. Inactive unless the marker file `.build-active` exists.
